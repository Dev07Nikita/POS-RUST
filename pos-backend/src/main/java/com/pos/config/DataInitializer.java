        ...
        public void run(String... args) {
                ...
                if (productRepository.count() == 0) {
                        productRepository.save(
                                        Product.builder().code("001").name("Tindi Coffee").price(250.0).costPrice(180.0)
                                                        .stockQuantity(50).category("Drinks").build());
                        productRepository.save(
                                        Product.builder().code("002").name("Premium Tea").price(150.0).costPrice(90.0)
                                                        .stockQuantity(80).category("Drinks").build());
                        productRepository.save(
                                        Product.builder().code("003").name("Glazed Donut").price(100.0).costPrice(45.0)
                                                        .stockQuantity(30).category("Snacks").build());
                        productRepository.save(
                                        Product.builder().code("004").name("Beef Burger").price(450.0).costPrice(280.0)
                                                        .stockQuantity(20).category("Food").build());
                        log.info("Seeded 4 sample products with cost data");
                }

                // No demo branches seeded. Add manual branches from the frontend.
        }
}
